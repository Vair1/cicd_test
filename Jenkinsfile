...
pipeline {
    ...
    parameters {
        ...
        string(defaultValue: "${env.baseConnString}", description: 'Путь подключения к базе', name: 'baseConnString')
            ...
            steps {
                timestamps {
                    script {
                        ...
                        returnCode = utils.cmd("runner vanessa --settings tools/vrunner.json ${platform1cLine} --ibconnection \"${baseConnString}\" ${admin1cUsrLine} ${admin1cPwdLine} --pathvanessa tools/add/bddRunner.epf")
